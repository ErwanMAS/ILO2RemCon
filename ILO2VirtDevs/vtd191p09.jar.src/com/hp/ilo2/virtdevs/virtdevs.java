...
{
  int dev_cd_device = 0; int dev_fd_device = 0; int unq_feature = 0;
  ...
  public void init()
  {
    ...
    if (str1 != null) {
      try {
        ...
        D.println(0, "Couldn't parse INFO0: " + localNumberFormatException1);
      }
    }
    try
    {
      ...
      D.println(0, "Couldn't parse INFO1: " + localNumberFormatException2);
    }
    
    this.configuration = getParameter("INFO2");
    if (this.configuration == null) { this.configuration = "auto";
    }
    ...
    try {
      ...
      D.println(0, "Couldn't parse UNIQUE_FEATURES: " + localNumberFormatException3);
    }
  ...
  public void stop() {
    ...
    if (this.fdConnection != null) {
      try {
        ...
        D.println(3, localIOException1.toString());
      }
    }
    if (this.cdConnection != null) {
      try {
        ...
        D.println(3, localIOException2.toString());
      }
    }
  }
  ...
  public boolean ui_init(String paramString, Image[] paramArrayOfImage) {
    ...
    for (int j = 0; (arrayOfString != null) && (j < arrayOfString.length); j++) {
      D.println(3, "init:deviceName = " + arrayOfString[j]);
  ...
  public void itemStateChanged(ItemEvent paramItemEvent) {
    ...
    if (paramItemEvent.getSource() == this.roCbox) {
      D.println(3, "Read only = " + this.roCbox.getState());
      if (this.fdConnection != null) {
  ...
  {
    D.println(3, "ActonPerformed " + paramActionEvent + " " + paramActionEvent.getSource());
    ...
    } else if (paramActionEvent.getSource() == this.fdChooseFile)
    {
      this.fdSelected = this.fdChooseFile.getText();
      if (this.fdThread != null) change_disk(this.fdConnection, this.fdSelected);
      D.println(3, "actionPerformed(2):fdSelected = " + this.fdSelected);
    } else if (paramActionEvent.getSource() == this.cdBrowse)
    {
      D.println(3, "actionPerformed:cdSelected = " + this.cdSelected);
      localVFileDialog = new VFileDialog("Choose CD/DVD-ROM Image File");
      str = localVFileDialog.getString();
      if (str != null)
        this.cdChooseFile.setText(str);
      this.cdSelected = this.cdChooseFile.getText();
      D.println(3, "FDIO.actionPerformed:cdSelected = " + this.cdSelected);
      ...
      D.println(3, "actionPerformed(2):cdSelected = " + this.cdSelected);
      if (this.cdThread != null) change_disk(this.cdConnection, this.cdSelected);
  ...
  public void do_floppy(String paramString)
  {
    if (!this.fdConnected) {
      ...
      int i;
      try { i = this.fdConnection.connect();
      ...
      try {
        ...
        D.println(0, "Exception during close: " + localException3);
      }
    }
  }
  ...
  void waitImage(Image paramImage, ImageObserver paramImageObserver)
  {
    ...
    int i;
    do { i = checkImage(paramImage, paramImageObserver);
      if ((i & 0xC0) != 0)
        break;
      Thread.yield();
  ...
  void setconfig(String paramString)
  {
    if (paramString.equals("floppy")) {
      ...
      this.cdStartButton.setEnabled(false);this.cdDriveList.setEnabled(false);
      this.cdch.setEnabled(false);
      if (cdimg_support) {
        this.cdChooseFile.setEditable(false);this.cdBrowse.setEnabled(false);
        this.cdCboxDev.setEnabled(false);this.cdCboxImg.setEnabled(false);
      }
    } else if (paramString.equals("cdrom")) {
      this.fdStartButton.setEnabled(false);this.fdCboxDev.setEnabled(false);
      this.fdCboxImg.setEnabled(false);this.fdDriveList.setEnabled(false);
      this.fdChooseFile.setEditable(false);this.fdBrowse.setEnabled(false);
      this.roCbox.setEnabled(false);
    ...
    {
      this.fdStartButton.setEnabled(true);this.fdCboxDev.setEnabled(true);
      this.fdCboxImg.setEnabled(true);
      this.fdChooseFile.setEditable(false);
      ...
      this.cdch.setEnabled(true);
      if (cdimg_support) {
        this.cdCboxDev.setEnabled(true);this.cdCboxImg.setEnabled(true);
        this.cdChooseFile.setEditable(false);this.cdBrowse.setEnabled(false);
      }
      if (this.fdCboxChecked == 0) {
        this.fdDriveList.setEnabled(true);
  ...
  public boolean rekey(String paramString)
  {
    ...
    try
    {
      D.println(3, "Downloading new key: " + this.base + paramString);
      ...
      D.println(0, "rekey: " + localException);
    ...
    try {
      ...
      D.println(0, "Couldn't parse new key: " + localNumberFormatException);
